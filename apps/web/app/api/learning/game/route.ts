...
export async function POST(request: Request) {
  ...
  if (!parsed.success) {
    return NextResponse.json({ error: "The game request was not valid bounded research context." }, { status: 400 });
  }
  try {
    const response = await createLearningGenerationProvider().generateVisualChallenge(parsed.data);
    ...
    return NextResponse.json({ error: "Local game generation failed safely." }, { status: 502 });
  }
}
